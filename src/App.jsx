...
export default function App({ session }) {
  ...
  async function submitForm() {
    ...
    setSaving(true);
    const payload = {
      user_id: userId, nom: form.nom, categorie: form.categorie,
      prix_achat: Number(form.prix_achat),
      prix_vente: form.prix_vente ? Number(form.prix_vente) : null,
      status: form.status, note: form.note, date: form.date,
    };
    if (editId) {
      const { error } = await supabase.from("items").update(payload).eq("id", editId).eq("user_id", userId);
      if (error) setError(error.message);
    } else {
    ...
    await fetchItems();
    setForm(EMPTY_FORM); setEditId(null); setSaving(false); setView("liste");
  }
  ...
  function startEdit(item) {
    setForm({ nom: item.nom, categorie: item.categorie, prix_achat: item.prix_achat, prix_vente: item.prix_vente || "", status: item.status, note: item.note || "", date: item.date || EMPTY_FORM.date });
    setEditId(item.id); setView("ajouter");
  }
  ...
  if (loading) return (
    <div style={{ background: "#0A0A0A", height: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#F0EDE8", fontFamily: "'DM Mono', monospace" }}>
      <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: "24px", fontWeight: 900 }}>FLIP<span style={{ color: "#FF6B35" }}>TRACK</span></div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'DM Mono','Courier New',monospace", background: "#0A0A0A", minHeight: "100vh", color: "#F0EDE8", paddingBottom: "80px" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}body{background:#0A0A0A}
        input,select,textarea{background:#141414;border:1px solid #2A2A2A;color:#F0EDE8;padding:10px 14px;border-radius:8px;font-family:inherit;font-size:14px;width:100%;outline:none;transition:border .2s}
        ...
        .tab-btn{padding:6px 14px;border-radius:20px;border:none;cursor:pointer;font-family:inherit;font-size:11px;font-weight:500;transition:all .2s;white-space:nowrap}
        .tab-btn.active{background:#FF6B35;color:#0A0A0A}.tab-btn.inactive{background:#1A1A1A;color:#666}
        ...
        .btn-primary{background:#FF6B35;color:#0A0A0A;border:none;padding:12px 24px;border-radius:10px;cursor:pointer;font-family:inherit;font-weight:600;font-size:14px;transition:opacity .2s}
        .btn-primary:hover{opacity:.85}.btn-primary:disabled{opacity:.5;cursor:not-allowed}
        .btn-ghost{background:transparent;color:#666;border:1px solid #2A2A2A;padding:8px 14px;border-radius:8px;cursor:pointer;font-family:inherit;font-size:12px;transition:all .2s}
        .btn-ghost:hover{color:#F0EDE8;border-color:#444}
        .nav-item{display:flex;flex-direction:column;align-items:center;gap:3px;cursor:pointer;padding:8px 20px;border-radius:12px;background:none;border:none;font-family:inherit;transition:color .2s;color:#444}
        ...
        .filter-row{display:flex;gap:6px;overflow-x:auto;padding-bottom:4px}.filter-row::-webkit-scrollbar{display:none}
      `}</style>

      {/* Header */}
      <div style={{ padding:"20px 20px 14px", borderBottom:"1px solid #1A1A1A", position:"sticky", top:0, background:"#0A0A0AEE", backdropFilter:"blur(10px)", zIndex:50, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:"20px", fontWeight:900 }}>FLIP<span style={{ color:"#FF6B35" }}>TRACK</span></div>
          <div style={{ fontSize:"10px", color:"#333", marginTop:"2px" }}>{userEmail}</div>
        </div>
        <div style={{ position:"relative" }}>
          <button onClick={() => setShowMenu(!showMenu)} style={{ background:"#1A1A1A", border:"1px solid #2A2A2A", borderRadius:"8px", padding:"8px 12px", cursor:"pointer", color:"#666", fontSize:"16px" }}>⚙️</button>
          {showMenu && (
            <div style={{ position:"absolute", right:0, top:"44px", background:"#141414", border:"1px solid #2A2A2A", borderRadius:"10px", padding:"8px", minWidth:"180px", zIndex:200 }}>
              <div style={{ fontSize:"11px", color:"#444", padding:"6px 10px 8px", borderBottom:"1px solid #2A2A2A", marginBottom:"4px" }}>{userEmail}</div>
              <button onClick={() => supabase.auth.signOut()} style={{ width:"100%", background:"none", border:"none", color:"#FF4E4E", padding:"8px 10px", textAlign:"left", cursor:"pointer", fontFamily:"inherit", fontSize:"13px", borderRadius:"6px" }}>
                🚪 Se déconnecter
              </button>
            </div>
          )}
      ...
      <div style={{ padding:"20px" }}>
          ...
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"20px" }}>
              <div className="stat-card" style={{ gridColumn:"1 / -1", background:benefice>=0?"#06D6A008":"#FF4E4E08", borderColor:benefice>=0?"#06D6A030":"#FF4E4E30" }}>
                ...
                <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:"34px", fontWeight:900, color:benefice>=0?"#06D6A0":"#FF4E4E" }}>{benefice>=0?"+":""}{formatEur(benefice)}</div>
                <div style={{ fontSize:"11px", color:"#444", marginTop:"6px" }}>ROI : {roiGlobal}% · {vendus.length} vendu{vendus.length>1?"s":""}</div>
              </div>
              {[
                { label:"INVESTI", val:formatEur(totalInvesti), color:"#FF6B35", sub:`${items.length} article${items.length>1?"s":""}` },
                { label:"EN VENTE", val:enVente.length, color:"#FFE66D", sub:`Stock : ${formatEur(valeurStock)}` },
                { label:"ENCAISSÉ", val:formatEur(totalVendu), color:"#06D6A0", sub:`${vendus.length} vendu${vendus.length>1?"s":""}` },
                { label:"EN STOCK", val:items.filter(i=>i.status==="achete").length, color:"#A8DADC", sub:"Pas encore listés" },
              ].map(s => (
                <div key={s.label} className="stat-card">
                  <div style={{ fontSize:"10px", color:"#666", marginBottom:"4px", letterSpacing:"1px" }}>{s.label}</div>
                ...
                return (
                  <div key={key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 0", borderBottom:"1px solid #1A1A1A" }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:"14px", fontWeight:700, color:catBenef>=0?"#06D6A0":"#FF4E4E" }}>{catBenef>=0?"+":""}{formatEur(catBenef)}</div>
                      <div style={{ fontSize:"10px", color:"#444" }}>{catVendus.length} vendu{catVendus.length>1?"s":""}</div>
                    </div>
                  </div>
                );
              })}
              {!items.length && <div style={{ textAlign:"center", color:"#444", padding:"30px 0", fontSize:"13px" }}>Aucun article — commence par en ajouter un !</div>}
            </div>
          ...
          <div>
            ...
            {!filteredItems.length && (
              <div style={{ textAlign:"center", color:"#444", padding:"40px 0", fontSize:"13px" }}>
                Aucun article<br/><button className="btn-primary" style={{ marginTop:"16px" }} onClick={() => setView("ajouter")}>+ Ajouter</button>
              </div>
            )}
              ...
              return (
                <div key={item.id} className="item-card">
                  ...
                  <div style={{ display:"flex", gap:"20px", marginBottom:"12px", flexWrap:"wrap" }}>
                    <div><div style={{ fontSize:"10px", color:"#555" }}>ACHETÉ</div><div style={{ fontSize:"17px", fontWeight:700, color:"#FF6B35" }}>{formatEur(item.prix_achat)}</div></div>
                    {item.prix_vente && <div><div style={{ fontSize:"10px", color:"#555" }}>VENDU</div><div style={{ fontSize:"17px", fontWeight:700, color:"#06D6A0" }}>{formatEur(item.prix_vente)}</div></div>}
                    {marge!==null && <div><div style={{ fontSize:"10px", color:"#555" }}>MARGE</div><div style={{ fontSize:"17px", fontWeight:700, color:marge>=0?"#06D6A0":"#FF4E4E" }}>{marge>=0?"+":""}{formatEur(marge)} <span style={{ fontSize:"11px", opacity:.7 }}>({roi}%)</span></div></div>}
                  </div>
                  {item.note && <div style={{ fontSize:"12px", color:"#555", marginBottom:"12px", fontStyle:"italic" }}>"{item.note}"</div>}
        ...
        {view === "ajouter" && (
          <div>
            <div style={{ fontFamily:"'Unbounded',sans-serif", fontSize:"15px", fontWeight:700, marginBottom:"24px" }}>{editId?"MODIFIER L'ARTICLE":"NOUVEL ARTICLE"}</div>
                ...
                <div style={{ display:"flex", gap:"8px" }}>
                  {Object.entries(STATUS).map(([k,v]) => (
                    <button key={k} onClick={() => setForm({...form,status:k})}
                      style={{ flex:1, padding:"10px 6px", borderRadius:"8px", border:`1px solid ${form.status===k?v.color:"#2A2A2A"}`, background:form.status===k?v.bg:"transparent", color:form.status===k?v.color:"#555", cursor:"pointer", fontSize:"11px", fontFamily:"inherit", fontWeight:500 }}>
                      {v.label}
                    </button>
                  ))}
                </div>
              ...
              {form.prix_achat && form.prix_vente && Number(form.prix_achat)>0 && (
                <div style={{ background:"#141414", borderRadius:"10px", padding:"14px", border:"1px solid #2A2A2A" }}>
                  ...
                  <div style={{ display:"flex", gap:"24px" }}>
                    {[
                      { label:"Marge", value:formatEur(Number(form.prix_vente)-Number(form.prix_achat)), color:(Number(form.prix_vente)-Number(form.prix_achat))>=0?"#06D6A0":"#FF4E4E" },
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"#0A0A0AF0", backdropFilter:"blur(12px)", borderTop:"1px solid #1A1A1A", display:"flex", justifyContent:"space-around", padding:"8px 0 12px", zIndex:100 }}>
        {[
          ...
          <button key={tab.id} className={`nav-item ${view===tab.id||(tab.id==="ajouter_new"&&view==="ajouter"&&!editId)?"active":""}`}
            onClick={() => { if(tab.id==="ajouter_new"){setEditId(null);setForm(EMPTY_FORM);setView("ajouter");}else setView(tab.id); }}>
