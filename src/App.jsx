...
export default function App({ session }) {
  ...
  function handleImport(data) {
    setForm(prev => ({
      ...
      prix_achat: data.prix ? String(data.prix) : prev.prix_achat,
      categorie: data.categorie || prev.categorie,
      note: data.plateforme ? `Importé depuis ${data.plateforme}${data.image ? " · Photo: " + data.image : ""}` : prev.note,
    }));
  }
    ...
    <div style={{ fontFamily: "'DM Mono', 'Courier New', monospace", background: "#0A0A0A", minHeight: "100vh", color: "#F0EDE8", paddingBottom: "80px" }}>
      ...
      <div style={{ padding: "20px 20px 14px", borderBottom: "1px solid #1A1A1A", position: "sticky", top: 0, background: "#0A0A0AEE", backdropFilter: "blur(10px)", zIndex: 50, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: "10px", color: "#333", marginTop: "2px" }}>{session.user.email}</div>
        </div>
        <button onClick={handleLogout} style={{ background: "none", border: "1px solid #2A2A2A", color: "#555", padding: "6px 12px", borderRadius: "8px", cursor: "pointer", fontFamily: "inherit", fontSize: "11px" }}>
          Déconnexion
        </button>
      </div>
      ...
      <div style={{ padding: "20px" }}>
        ...
        {view === "ajouter" && (
          <div>
            <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: "15px", fontWeight: 700, marginBottom: "24px" }}>
              {editId ? "MODIFIER L'ARTICLE" : "NOUVEL ARTICLE"}
            </div>
            {!editId && <ImportURL onImport={handleImport} />}
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#0A0A0AF0", backdropFilter: "blur(12px)", borderTop: "1px solid #1A1A1A", display: "flex", justifyContent: "space-around", padding: "8px 0 12px", zIndex: 100 }}>
        {[
          ...
          { id: "prix", emoji: "💰", label: "Prix" },
          { id: "assistant", emoji: "🤖", label: "IA" },
        ].map(tab => (
          <button key={tab.id} className={`nav-item ${view === tab.id || (tab.id === "ajouter_new" && view === "ajouter" && !editId) ? "active" : ""}`}
            ...
            <span style={{ fontSize: tab.id === "ajouter_new" ? "22px" : "18px" }}>{tab.emoji}</span>
